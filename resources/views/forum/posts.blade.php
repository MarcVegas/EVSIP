    ...
    @if (count($posts) > 0)
        @foreach ($posts as $post)
            <div class="ui raised inverted segment">
                ...
                <small>{{ ($post->title != NULL) ? 'Posted' : 'Commented' }} on: {{date('d M y, h:i a', strtotime($post->created_at))}}</small>
                ...
                @else 
                    <h5>You left a comment on this <a href="/forum/{{$post->post_id-1}}">post</a></h5>
                @endif
            </div>
        @endforeach
    @else 
        <div class="ui basic center aligned segment">
            ...
            <h4>You currently have nothing posted</h4>
            <p>Engage with the community with a question or discussion by clicking <strong>Create Post</strong></p>
        </div>
    @endif
</div>
@endsection